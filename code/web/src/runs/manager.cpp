...
runs::manager::create(const std::uint_fast64_t repository,
    ...
    const auto result = tx.prepared(sql)(repository)(commit_hash).exec();
...
runs::manager::find(std::uint_fast64_t id) const {
    ...
    const std::string sql(
            "SELECT id,repository,created_at FROM runs WHERE id=$1 LIMIT 1");
    const auto result = tx.prepared(sql)(id).exec();
    ...
    if (!result.empty()) {
        ...
        return std::make_optional(std::shared_ptr<runs::run>(new runs::run(
                ...
                result[0]["repository"].as<std::uint_fast64_t>()
        )));
    }
...
runs::manager::find_order(const std::uint_fast64_t run) const {
    ...
    const auto result = tx.prepared(sql)(run).exec();
