...
ggplot(data, aes(idx)) +
  ...
  geom_line(aes(y = GreedyCoverAll, color = "Greedy")) +
  ggtitle("Performance of the Greedy algorithm on Stratego") +
  labs(x = "Test run", y = "# test cases before first failure") +
  scale_color_manual(name = "Algorithm", values = c("Original" = "gray", "Greedy" = "red")) +
